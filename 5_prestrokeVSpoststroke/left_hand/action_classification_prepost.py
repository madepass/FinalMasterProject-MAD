...
#%% 
os.chdir('D:\\ub_neuroComp\\dancause_data\\processing\\4_preVSpost\\left\\export')
#%% general info
...
for i_sub in subjects:
    ...
    RFE_FC = skfs.RFE(c_MLR, n_features_to_select=15)
    ...
    if 0:
        ...
        pp.title(freq_band)
        pp.savefig(res_dir+'support_net_RFE_'+freq_band+'_'+measure_label + '.' + fmt_grph)
        pp.close()
    ...
    def plot_multiclass_roc(clf, X_test, y_test, n_classes, figsize=(17, 6), save_text = True, legend = True):
        ...
        pp.close()
    plot_multiclass_roc(c_MLR_original, vect_features[ind_test_original,:], labels[ind_test_original], n_classes=n_motiv, figsize=(4, 3), legend = False)
